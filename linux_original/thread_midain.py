...
def best_balance_parameter(data,number_of_points = 50,name=""):
    ...
    # epsilon_array = np.hstack((np.linspace(0.05,0.5,10),np.linspace(1,5,10)))
    epsilon_array = np.linspace(0.05, 0.5, 10)
    ...
    # f.close()
    print "constand:",c_array[0][index_min]
...
if __name__ == '__main__':
    ...
    np.savetxt("./result_sample_aggreation.txt", result_sample_aggreation)
    np.savetxt("./result_personlized_sample.txt", result_personlized_sample)
    np.savetxt("./result_laplace.txt", result_laplace)
    ...
    plt.tight_layout()
    plt.savefig('../figure/thread_median.png',dpi=600, bbox_extra_artists=(),bbox_inches='tight')
    plt.show()
