...
def sample_aggreation(data,epsilon = 1.0,delta = 0.00001,number_of_partition = 5,beta = 0.5):
    ...
    for loop in np.arange(0, number_of_partition, 1):
        ...
        z_array[loop] = np.true_divide(np.std(sampeld_data),np.average(sampeld_data))
    ...
    real_result = np.true_divide(np.std(data),np.average(data))
...
def personlized_sample_aggregation(data, epsilon=1.0, delta=0.00001, number_of_partition=5, beta=0.5, constant=2.0):
    # number_of_partition = 5
    # beta = 0.5
    # epsilon = 1.0
    # delta = 0.00001
    ...
    record_sensitivity = np.array([0.0 for a in np.arange(0, len(data), 1)])
    for index in np.arange(0, len(data), 1):
        tmp_data = data
        tmp_data = np.delete(tmp_data, index, axis=0)
        cv_data = np.true_divide(np.std(data), np.average(data))
        cv_tmp_data = np.true_divide(np.std(tmp_data), np.average(tmp_data))
        record_sensitivity[index] = np.abs(cv_data-cv_tmp_data)
    ...
    for round in np.arange(0, len(record_sensitivity), 1):
        if record_sensitivity[round] > constant:
            sample_probability[round] = np.true_divide((1 - np.exp(epsilon)), (1 - np.exp(epsilon * np.true_divide(record_sensitivity[round], constant))))
        else:
            sample_probability[round] = 1
        # print "**sample_probability[round]:",sample_probability[round]
    ...
    for loop in np.arange(0, number_of_partition, 1):
        ...
        while len(sampled_data) == 0:
            ...
            sampled_data = random_pick_probability(data, sample_probability)
        z_array[loop] = np.true_divide(np.std(sampled_data), np.average(sampled_data))
...
def personlized_sample(data, epsilon=1.0, delta=0.00001, number_of_partition=1, beta=0.5, constant=2.0):
    ...
    sampled_record_sensitivity = np.array([0.0 for a in np.arange(0, len(sampled_data), 1)])
    for index in np.arange(0, len(sampled_data), 1):
        tmp_data = sampled_data
        tmp_data = np.delete(tmp_data, index, axis=0)
        cv_data = np.true_divide(np.std(sampled_data), np.average(sampled_data))
        cv_tmp_data = np.true_divide(np.std(tmp_data), np.average(tmp_data))
        sampled_record_sensitivity[index] = np.abs(cv_tmp_data-cv_data)
    # print "sampled_record_sensitivity:",sampled_record_sensitivity

    local_sensitivity = np.max(sampled_record_sensitivity)
...
def laplace_mechaism(data,epsilon = 1.0,delta = 0.00001,number_of_partition = 5,beta = 0.5,constant=2):
    ...
    for index in np.arange(0, len(data), 1):
        ...
        cv_tmp_data = np.true_divide(np.std(tmp_data), np.average(tmp_data))
        record_sensitivity[index] = np.abs(cv_data-cv_tmp_data)
    local_sensitivity = np.max(record_sensitivity)
...
####################################################################
def best_balance_parameter(data,number_of_points = 20,number_data=100):
    # loop = 500
    loop = 500
    record_sensitivity = np.array([0.0 for a in np.arange(0, len(data), 1)])
    for index in np.arange(0, len(data), 1):
    ...
    # print "record sensitivity:",record_sensitivity
    ...
    if not os.path.exists(path):
        os.mkdir(path)
    np.savetxt(path+str(number_data)+"real_cv_epsilon.txt",epsilon_array)
    np.savetxt(path+str(number_data)+"real_cv_c_array.txt", c_array)
    np.savetxt(path+str(number_data)+"real_cv_error.txt", error)

    for angle in range(0, 360, 10):
        # ax.set_zlabel("Angle: " + str(angle))
        ax.view_init(30, angle)
        filename = "./real_data" +"number_data_"+str(number_data) +  str(angle) + ".png"
        # plt.show()
        plt.savefig(path + filename)
    f = open(path+str(number_data)+"best_cv.txt", "w")
    f.write("best balance parameter(real_data_cv function) ="+str(c_array[0][index_min]))
    f.close()
...
# but also related to privacy budget.
####################################################################
def best_balance_parameter_for_aggregation(data,number_of_points = 20,number_data=100):
    # loop = 500
    loop = 500
    record_sensitivity = np.array([0.0 for a in np.arange(0, len(data), 1)])
    for index in np.arange(0, len(data), 1):
    ...
    for index1 in np.arange(0,len(c_array),1):
        ...
        for index2 in np.arange(0,len(c_array[0]),1):
            tmp_error = 0
            for count in np.arange(0, loop, 1):
                tmp = personlized_sample_aggregation(data, epsilon=epsilon_array[index1][index2], constant=c_array[index1][index2])
                tmp_error = tmp_error + np.square(tmp[0] - tmp[1])
            # print "tmp_error:",tmp_error
            error[index1][index2] = np.sqrt(np.true_divide(tmp_error, loop))
    ...
    # plt.subplots_adjust(left=0.18, wspace=0.25, hspace=0.25,bottom=0.13, top=0.91)
    # plt.tight_layout()
    path = "../figure/best_balance_parameter_cv/personalized_sample_and_aggregation/"
    if not os.path.exists(path):
        os.mkdir(path)
    np.savetxt(path+"real_cv_epsilon.txt",epsilon_array)
    np.savetxt(path+"real_cv_c_array.txt", c_array)
    np.savetxt(path+"real_cv_error.txt", error)
...
# data_loaded = load_data(f='../data/OP_DTL_GNRL_PGYR2019_P06302020_dataset.csv',index=0,number_data=500);
# constant = best_balance_parameter(data,40,len(data_loaded))
#
...
if not os.path.exists(path):
    os.mkdir(path)
epsilon_array_aggregation = np.loadtxt(path+"real_cv_epsilon.txt")
c_array_aggregation = np.loadtxt(path+"real_cv_c_array.txt" )
error_aggregation = np.loadtxt(path+"real_cv_error.txt" )


path = "../figure/best_balance_parameter_cv/"
number_data = 209
epsilon_array = np.loadtxt(path+str(number_data)+"real_cv_epsilon.txt")
c_array = np.loadtxt(path+str(number_data)+"real_cv_c_array.txt")
error = np.loadtxt(path+str(number_data)+"real_cv_error.txt")
#
# epsilon = epsilon_array[0:,1]
# print "epsilon_array:",epsilon
#
# print "error:",error
#
# print "error:",error[0]
# # tmp = np.array(error[0])
#
#
# print "c_array:",c_array
# print "index:",np.argmin(error[0])
...
for index_1 in np.arange(0,len(privacy_array),1):
    ...
    for index_2 in np.arange(0,number_repeate,1):
        ...
        # print "personlized sample 1 time:"
        tmp = personlized_sample_aggregation(data, epsilon=privacy_array[index_1], constant=constant_aggregation)
...
# plt.title('comparison of error for coefficient of variation',fontsize=font_size)
...
plt.savefig('../figure/cpu_real_data_coefficient_variation.png',dpi=600, bbox_extra_artists=(),bbox_inches='tight',figsize=(4.0, 2.0))
plt.show()
...
#     os.mkdir(path)
#
# plt.savefig('../figure/real_data_coefficient_variation.png',dpi=600, bbox_extra_artists=(),bbox_inches='tight',figsize=(4.0, 2.0))
# plt.show()
#
#
#
