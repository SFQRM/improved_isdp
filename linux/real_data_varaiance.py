...
def sample_aggreation(data,epsilon = 1.0,delta = 0.00001,number_of_partition = 5,beta = 0.5):
    ...
    for loop in np.arange(0, number_of_partition, 1):
        ...
        z_array[loop] = np.std(sampeld_data)
...
def personalized_sample_aggregation(data, epsilon=1.0, delta=0.00001, number_of_partition=5, beta=0.5, constant=2.0):
    ...
    for index in np.arange(0, len(data), 1):
        ...
        record_sensitivity[index] = np.abs(np.std(data) - np.std(tmp_data))
    ...
    for loop in np.arange(0, number_of_partition, 1):
        ...
        while len(sampled_data) == 0:
            ...
            sampled_data = random_pick_probability(data, sample_probability)
        z_array[loop] = np.std(sampled_data)
...
def personlized_sample(data, epsilon=1.0, delta=0.00001, number_of_partition=1, beta=0.5, constant=2.0):
    ...
    real_result = np.std(sampled_data)
    ...
    for index in np.arange(0, len(sampled_data), 1):
        ...
        tmp_data = np.delete(tmp_data, index, axis=0)
        sampled_record_sensitivity[index] = np.abs(np.std(sampled_data) - np.std(tmp_data))
...
def best_balance_parameter(data,number_of_points = 50,number_data=100):
    ...
    # epsilon_array = np.hstack((np.linspace(0.01,0.5,10), np.linspace(0.5, 1, 5)))
    ...
    path = "../figure/best_balance_parameter_standard_deviation/"
    if not os.path.exists(path):
        os.mkdir(path)
    np.savetxt(path + str(number_data)+"real_varaince_epsilon.txt", epsilon_array)
    np.savetxt(path + str(number_data)+"real_varaince_c_array.txt", c_array)
    np.savetxt(path + str(number_data)+"real_varaince_error.txt", error)
    ...
    my_x_ticks = np.linspace(np.min(record_sensitivity),np.max(record_sensitivity),5)
    ...
    for angle in range(0, 360, 10):
        ...
        # plt.show()
        plt.savefig("../figure/best_balance_parameter_standard_deviation/" + filename)
    f = open("../figure/best_balance_parameter_standard_deviation/"+str(number_data)+"real_data_best_balance_variance.txt", "w")
    f.write("best balance parameter(real data standard deviation function) ="+str(c_array[0][index_min]))
    f.close()
...
def best_balance_parameter_for_aggregation(data,number_of_points = 50,number_data=100):
    ...
    path = "../figure/best_balance_parameter_standard_deviation/personalized_sample_aggregation/"
    if not os.path.exists(path):
        os.mkdir(path)
    np.savetxt(path +"real_varaince_epsilon.txt", epsilon_array)
    np.savetxt(path +"real_varaince_c_array.txt", c_array)
    np.savetxt(path +"real_varaince_error.txt", error)
...
if not os.path.exists(path):
    os.mkdir(path)
epsilon_array_aggregation = np.loadtxt(path +"real_varaince_epsilon.txt")
c_array_aggregation = np.loadtxt(path +"real_varaince_c_array.txt")
error_aggregation = np.loadtxt(path +"real_varaince_error.txt")

path = "../figure/best_balance_parameter_standard_deviation/"
number_data = 209
epsilon_array = np.loadtxt(path + str(number_data) + "real_varaince_epsilon.txt")
c_array = np.loadtxt(path + str(number_data) + "real_varaince_c_array.txt")
error = np.loadtxt(path + str(number_data) + "real_varaince_error.txt")
...
# plt.title('comparison of error for standard deviation function',fontsize=font_size )
...
plt.tight_layout()
plt.savefig('../figure/cpu_real_data_standard_deviation.png',dpi=600, bbox_extra_artists=(),bbox_inches='tight',figsize=(4.0, 2.0))
plt.show()
