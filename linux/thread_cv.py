...
####################################################################
def best_balance_parameter(data,number_of_points = 10,name=""):
    # loop = 100
    # print "best_banlance_parameter bigen:"
    loop = 100
    record_sensitivity = np.array([0.0 for a in np.arange(0, len(data), 1)])
    for index in np.arange(0, len(data), 1):
        tmp_data = data
    ...
    # return np.average(record_sensitivity) + (np.max(record_sensitivity) - np.average(record_sensitivity))*0.8
    ...
    # epsilon_array = np.hstack((np.linspace(0.5,1,5),np.linspace(1,5,10)))
    # epsilon_array = np.hstack((np.linspace(0.1,0.5,10),np.linspace(1,5,10)))
    ...
    for index1 in np.arange(0,len(c_array),1):
        # print "name:",name,"index1:",index1, "len(c_array):",len(c_array),"\ln"
    ...
    #     # plt.show()
    #     plt.savefig("../figure/best_balance_parameter_cv" + filename)
    # f = open("../figure/best_balance_parameter_cv/best_cv.txt", "w")
    # f.write("best balance parameter(cv function) ="+str(c_array[0][index_min]))
    # f.close()
    print "best constant:",c_array[0][index_min]
...
# but also related to privacy budget.
####################################################################
def best_balance_parameter_for_aggregation(data,number_of_points = 10,name=""):
    # loop = 100
    # print "best_banlance_parameter bigen:"
    loop = 100
    record_sensitivity = np.array([0.0 for a in np.arange(0, len(data), 1)])
...
def one_data_set(epsilon, mean, standard_deviation, number_of_number, repeate, number_of_thread):
    ...
    for round in np.arange(0, repeate, 1):
        ...
        tmp = personalized_sample_aggregation(data, epsilon=epsilon, constant=constant_aggregation)
    ...
    # print "result:",[result_sample_aggreation, result_personlized_sample, result_laplace, result_personalized_sample_aggregation]
...
if __name__ == '__main__':
    ...
    for index_1 in np.arange(0, len(privacy_array), 1):
        ...
        # print "result_personslized_sample_aggregation:", result_personalized_sample_aggregation[index_1]
    ...
    # print "result_personslized_sample_aggregation:", result_personalized_sample_aggregation
    ...
    plt.tight_layout()
    plt.savefig('../figure/thread_coefficient_variation.png',dpi=600, bbox_extra_artists=(),bbox_inches='tight')
    plt.show()
