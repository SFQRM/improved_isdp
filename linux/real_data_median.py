...
# aggreation funciton is average
####################################################################
def sample_aggreation(data,epsilon = 1.0,delta = 0.00001,number_of_partition = 5,beta = 0.5):
    ...
    for loop in np.arange(0, number_of_partition, 1):
        ...
        z_array[loop] = np.median(sampeld_data)
...
def personalized_sample_aggregation(data, epsilon=1.0, delta=0.00001, number_of_partition=5, beta=0.5, constant=2.0):
    ...
    for index in np.arange(0, len(data), 1):
        ...
        record_sensitivity[index] = np.abs(np.median(data)-np.median(tmp_data))
    ...
    for loop in np.arange(0, number_of_partition, 1):
        ...
        while len(sampled_data) == 0:
            ...
            sampled_data = random_pick_probability(data, sample_probability)
        z_array[loop] = np.median(sampled_data)
...
def personlized_sample(data, epsilon=1.0, delta=0.00001, number_of_partition=1, beta=0.5, constant=2.0):
    ...
    real_result = np.median(sampled_data)
    # print "data:",data
    ...
    for index in np.arange(0, len(sampled_data), 1):
        ...
        tmp_data = np.delete(tmp_data, index, axis=0)
        sampled_record_sensitivity[index] = np.abs(np.median(sampled_data) - np.median(tmp_data))
...
def best_balance_parameter(data,number_of_points = 50,number_data=100):
    ...
    for index in np.arange(0, len(data), 1):
        ...
        record_sensitivity[index] = np.abs(np.median(data) - np.median(tmp_data))
        # print "record_sensitivity[index] :",record_sensitivity[index]
    ...
    path = "../figure/best_balance_parameter_median/"
    if not os.path.exists(path):
        os.mkdir(path)
    np.savetxt(path +str(number_data)+ "real_median_epsilon.txt", epsilon_array)
    np.savetxt(path +str(number_data)+ "real_median_array.txt", c_array)
    np.savetxt(path +str(number_data)+  "real_median_error.txt", error)
    ...
    for angle in range(0, 360, 10):
        ...
        filename = "./real_data" +"data_number"+str(number_data)+ str(angle) + ".png"
        # plt.show()
        plt.savefig("../figure/best_balance_parameter_median/" + filename)
    f = open("../figure/best_balance_parameter_median/"+str(number_data)+ "real_data_best_balance_median.txt","w")
    f.write("best balance parameter(real_data_median function) ="+str(c_array[0][index_min]))
    f.close()
...
def best_balance_parameter_for_aggregation(data,number_of_points = 50,number_data=100):
    ...
    path = "../figure/best_balance_parameter_median/personalized_sample_aggregaiton/"
    if not os.path.exists(path):
        os.mkdir(path)
    np.savetxt(path + "real_median_epsilon.txt", epsilon_array)
    np.savetxt(path + "real_median_array.txt", c_array)
    np.savetxt(path +  "real_median_error.txt", error)
...
# constant = best_balance_parameter_for_aggregation(data,20)
...
for index_1 in np.arange(0,len(privacy_array),1):
    ...
    for index_2 in np.arange(0,number_repeate,1):
        ...
        tmp = personalized_sample_aggregation(data, epsilon=privacy_array[index_1], constant=constant)
...
# plt.title('comparison of error for medain function',fontsize=font_size)
...
plt.tight_layout()
plt.savefig('../figure/cpu_real_data_median.png',dpi=600, bbox_extra_artists=(),bbox_inches='tight',figsize=(4.0, 2.0))
plt.show()
