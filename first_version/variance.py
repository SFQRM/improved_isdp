...
def best_balance_parameter(data,number_of_points = 50):
    ...
    c_array = np.linspace(np.median(record_sensitivity),np.max(record_sensitivity),number_of_points)
    ...
    f = open("figure/best_balance_parameter_standard_deviation/best_balance_variance.txt", "w")
    f.write("best balance parameter(standard deviation function) ="+str(c_array[0][index_min]))
    f.close()
...
path = "./figure/best_balance_parameter_standard_deviation/"
epsilon_array = np.loadtxt(path +   "real_varaince_epsilon.txt")
c_array = np.loadtxt(path +   "real_varaince_c_array.txt")
error = np.loadtxt(path +  "real_varaince_error.txt")
...
# plt.title('comparison of error for standard deviation function')
...
plt.tight_layout()
plt.savefig('./figure/standard_deviation.png')
plt.show()
