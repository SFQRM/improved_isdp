...
def personlized_sample_aggreation(data,epsilon = 1.0,delta = 0.00001,number_of_partition = 5,beta = 0.5,constant=2):
    # number_of_partition = 5
    # beta = 0.5
    # epsilon = 1.0
    # delta = 0.00001
    ...
    for loop in np.arange(0, number_of_partition, 1):
        ...
        sample_probability = np.true_divide((1 - np.exp(epsilon)),
                                            (1 - np.exp(np.true_divide(epsilon * record_sensitivity, constant))))
...
def best_balance_parameter(data,number_of_points = 50):
    ...
    ax.set_xlabel("privacy budget", fontsize=15)
    ax.set_ylabel("balance parameter", fontsize=15)
    ...
    for angle in range(0, 360, 10):
        ...
        filename = "./real_data_" + str(angle) + ".png"
        # plt.show()
        plt.savefig("../figure/best_balance_parameter_average/" + filename)
    f = open("../figure/best_balance_parameter_average/real_data_best_balance_average.txt","w")
    f.write("best balance parameter(average function) ="+str(c_array[0][index_min]))
    f.close()
    return c_array[0][index_min]

def load_data(f="../data/machine.data",index = 6):
    data = []
    data_file = open(f,"r")
...
data_loaded = load_data(f='../data/machine.data',index=6);
...
fig = plt.figure()
plt.plot(privacy_array, result_sample_aggreation, 'r-', linewidth=2, label='sample and aggreation sample')
...
plt.tight_layout()
plt.savefig('../figure/real_data_average.png',dpi=600, bbox_extra_artists=(),bbox_inches='tight')
plt.show()
