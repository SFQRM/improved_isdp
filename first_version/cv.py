...
####################################################################
def best_balance_parameter(data,number_of_points = 20):
    # loop = 200
    loop = 10
    record_sensitivity = np.array([0.0 for a in np.arange(0, len(data), 1)])
    for index in np.arange(0, len(data), 1):
...
# constant = 13.354417692422324
data = np.random.normal(mean, standard_deviation, number_of_number)
constant = best_balance_parameter(data, 20)

path = "./figure/best_balance_parameter_cv/"
epsilon_array = np.loadtxt(path+ "real_cv_epsilon.txt")
c_array = np.loadtxt(path+ "real_cv_c_array.txt")
error = np.loadtxt(path+ "real_cv_error.txt")
...
for index_1 in np.arange(0,len(privacy_array),1):
    ...
    for index_2 in np.arange(0,number_repeate,1):
        ...
        tmp = personlized_sample_aggregation(data, epsilon=privacy_array[index_1], constant=constant)
...
# plt.title('comparison of error for coefficient of variation')
...
plt.tight_layout()
plt.savefig('./figure/coefficient_variation.png')
plt.show()
